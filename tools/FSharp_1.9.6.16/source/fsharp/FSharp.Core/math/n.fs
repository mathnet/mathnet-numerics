...
#endif
    let max_int a b = if a<b then b else (a:int)  
    let min_int a b = if a<b then a else (b:int)  
...
//--------------------------------------------------------------------------
    ...
    let restrictTo d n =
       { bound = min_int d n.bound; digits = n.digits}
...
//--------------------------------------------------------------------------
    ...
    let add p q =
        let rbound = 1 + max_int p.bound q.bound
    ...
    let sub p q =
        ...
        let rbound = max_int p.bound q.bound
        let r = createN rbound
        let carry = 0
...
//--------------------------------------------------------------------------
    ...
    // REQUIRE: baseN + baseN.2^32 < Int64.max_int 
...
//--------------------------------------------------------------------------
    ...
    // Require "bits(res(i)) <= max_bits_inside_fp",
    // So choose "bits(K) + 2L <= max_bits_inside_fp".
    //
    ...
    let mkEncoding (bigL,k,bigK,bigN) =
      ...
      check (2 * bigL + k <= max_bits_inside_fp) "constraint";
#endif
      { bigL      = bigL;
    ...
    let calculateTableTow bigL = 
      // Given L.
      // Have L via "log2 K <= max_bits_inside_fp - 2L".
      // Have N via "N = K.L"
      // 
      let k    = max_bits_inside_fp - 2*bigL
      let bigK = pow64 2L k
    ...
    let encodePoly enc n =
      ...
      // with 0 <= pi < 2^bigL.
      //
      let poly = Array.create enc.bigK (fp_of_int 0)
    ...
    let decodeResultBits enc (poly : fp array) =
      ...
      //        <= 2^max_bits_inside_fp * (1 + X + ... X^n)
      //        <= 2^max_bits_inside_fp * (X^(n+1) - 1) / (X - 1)
      //        <= 2^max_bits_inside_fp * X^(n+1)       / (X - 1)
      //        <= 2^max_bits_inside_fp * X^(n+1)       / (X/2)     provided X/2 <= X-1
      //        <= 2^max_bits_inside_fp * X^n * 2
      //        <= 2^max_bits_inside_fp * (2^bigL)^n * 2
      //        <= 2^(max_bits_inside_fp + bigL.n + 1)
      //
      let mutable n = 0
      for i = 0 to poly.Length-1 do 
        if poly.[i] <> mzero then n <- i 
      done;
      let rbits = max_bits_inside_fp + enc.bigL * n + 1
    ...
    let decodePoly enc poly =
      ...
      // Note, 0 <= pi < max_fp.
      //
      let rbound = enc.bigN + boundInt // gross bound 
      ...
      let rec evaluate i j d =
        ...
        if i=enc.bigK then
          ...
          ( if j >= rbound then 
              ...
              let x = int64 (int_of_fp poly.[i]) * twopowersI64.[d]
...
//--------------------------------------------------------------------------
    ...
    let rec mul p q =
        let pq_bound = p.bound + q.bound
        if pq_bound < productDigitsUpperSchoolBook ||
        ...
        then
          ...
          if pq_bound < productDigitsUpperFft then
    ...
    let to_string n =
      ...
      let rec collect isLeading digits n = function
        | [] ->
            ...
            let n = eval n
#if CHECKED
            if checks then check (0 <= n) "to_string: digit0";
            if checks then check (n <= 9) "to_string: digit9";
#endif
            if isLeading && n=0 then digits // suppress leading 0  
            else string_of_int n :: digits
        | (k,ten2k)::prior ->
#if CHECKED
      ...
      match digits with 
      | [] -> "0" 
      | _ -> digits |> Array.of_list |> System.String.Concat 
            ...
            if i=len then
              acc
                ...
                if 0 <= d && d <= 9 then
                    ...
                    invalidArg "str" "the string was not a valid number"
       
        build (embed 0) 0

    let is_small n = (n.bound <= 1)
    let get_small n = index n 0
