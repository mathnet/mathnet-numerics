...
#light
#nowarn "35" // This construct is deprecated: This form of object expression is deprecated. Use 'member this.MemberName ... = ...' to define member implementations in object expressions.
...
let GetSingleCharAlphabet() = 
    if !unicode 
    then Set.of_list [ for c in 0..numLowUnicodeChars-1 do yield (char c)
                       for c in GetSpecificUnicodeChars() do yield c ]
    else Set.of_list [ for x in 0..255 ->  (char x) ]
         
let GetAlphabet() = 
    if !unicode 
    then Set.of_list [ for c in GetSingleCharAlphabet() do yield EncodeChar c
                       for uc in 0 .. NumUnicodeCategories-1 do yield EncodeUnicodeCategoryIndex uc ]
    else Set.of_list [ for c in GetSingleCharAlphabet() do yield EncodeChar c ]
...
type NfaNodeMap() = 
    ...
    member x.NewNfaNode(trs,ac) = 
        ...
        let node = {new NfaNode with Id=nodeId and Name=string nodeId and Transitions=trDict and Accepted=ac}
        map.[nodeId] <-node;
        node
...
let LexerStateToNfa (macros: Map<string,_>) (clauses: Clause list) = 
    ...
    let rec CompileRegexp re dest = 
        ...
        | Seq res -> 
            List.fold_right (CompileRegexp) res dest 
        | Inp (Alphabet c) -> 
...
type NfaNodeIdSet(nodes: NfaNodeIdSetBuilder) = 
    let s = nodes |> Seq.to_array |> Array.sort 
    member x.Representation = s
    member x.Elements = s 
    member x.Fold f z = Array.fold_left f z s
...
let NfaToDfa (nfaNodeMap:NfaNodeMap) nfaStartNode = 
    ...
    let EClosure (moves:list<NodeId>) = 
        let acc = new NfaNodeIdSetBuilder(10)
        for i in moves do
    ...
    let dfaNodes = ref (Map<NfaNodeIdSet,DfaNode>.Empty)
    ...
    let doneSet = ref NodeSetSet.Empty
...
let Compile spec = 
    List.fold_right
        (fun (name,args,clauses) (perRuleData,dfaNodes) -> 
            let nfa, actions, nfaNodeMap = LexerStateToNfa (Map.of_list spec.Macros) clauses
