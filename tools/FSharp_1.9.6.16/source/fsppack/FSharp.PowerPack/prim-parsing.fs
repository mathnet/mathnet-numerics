        ...
        while not finished do                                                                                    
            if stateStack.IsEmpty then 
...
#if DEBUG
                if debug then (Console.Write("{0} value(state), state ",valueStack.Count); stateStack.PrintStack())
#endif
                let action = 
                ...
                if kind = shiftFlag then (
...
#if DEBUG
                        if debug then Console.WriteLine("shifting, reduced errorRecoverylevel to {0}\n", errorSuppressionCountDown);
#endif
                    let nextState = actionValue action                                     
                    ...
                    stateStack.Push(nextState);                                                                
#if DEBUG
                    if debug then Console.WriteLine("shift/consume input {0}, shift to state {1}", report haveLookahead lookaheadToken, nextState);
#endif
                    haveLookahead <- false
...
#if DEBUG
                          if debug then Console.WriteLine("RecoverableParseErrorException...\n");
#endif
...
#if DEBUG
                    if debug then printfn "error on token '%A' " (if haveLookahead then Some(lookaheadToken) else None);
#endif
                    if errorSuppressionCountDown > 0 then 
...
#if DEBUG
                            if debug then printfn "poppin stack, lokking to shift both 'error' and that token, during end-of-file error recovery" ;
#endif
...
#if DEBUG
                        if debug then printfn "discarding token '%A' during error suppression" (if haveLookahead then Some(lookaheadToken) else None);
#endif
                        // Discard the token
                        haveLookahead <- false
                        ...
                        let explicit = Set.of_list [ for (tag,action) in actions -> tag ]
