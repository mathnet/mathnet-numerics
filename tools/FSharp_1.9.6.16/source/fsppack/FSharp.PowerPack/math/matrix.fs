...
namespace Microsoft.FSharp.Math
...
#endif
        member m.IsDense = match m with DenseRepr mr -> true | SparseRepr mr -> false

#if FX_NO_DEBUG_DISPLAYS
#else
        [<DebuggerBrowsable(DebuggerBrowsableState.Collapsed)>]
#endif
        member m.IsSparse = match m with DenseRepr mr -> false | SparseRepr mr -> true

#if FX_NO_DEBUG_DISPLAYS
#else
        [<DebuggerBrowsable(DebuggerBrowsableState.Collapsed)>]
#endif
        member m.InternalSparseColumnValues = match m with DenseRepr mr -> invalidOp "not a sparse matrix" | SparseRepr mr -> mr.SparseColumnValues

#if FX_NO_DEBUG_DISPLAYS
#else
        [<DebuggerBrowsable(DebuggerBrowsableState.Collapsed)>]
#endif
        member m.InternalSparseRowOffsets = match m with DenseRepr mr -> invalidOp "not a sparse matrix" | SparseRepr mr -> mr.SparseRowOffsets

#if FX_NO_DEBUG_DISPLAYS
#else
        [<DebuggerBrowsable(DebuggerBrowsableState.Collapsed)>]
#endif
        member m.InternalSparseValues = match m with DenseRepr mr -> invalidOp "not a sparse matrix" | SparseRepr mr -> mr.SparseValues

#if FX_NO_DEBUG_DISPLAYS
#else
        [<DebuggerBrowsable(DebuggerBrowsableState.Collapsed)>]
#endif
        member m.InternalDenseValues = match m with DenseRepr mr -> mr.Values | SparseRepr mr -> invalidOp "not a dense matrix" 
    ...
    module GenericImpl = 
        ...
        let inline getArray2  (arrDM : _[,]) i j   = arrDM.[i,j]
        let inline setArray2  (arrDM  : _[,]) i j x = arrDM.[i,j] <- x

        let inline createArray m = Array.zeroCreate m

        let inline createArray2 m n = Array2D.zeroCreate m n
        ...
        let inline assignDenseMatrixGU f (a:DenseMatrix<_>) = 
            assignArray2 a.NumRows a.NumCols f a.Values
        ...
        let initSparseMatrixGU maxi maxj ops s = 
            ...
            let colsA,valsA = 
               ...
               for i = 0 to maxi-1 do 
                  match tab.[i] with 
                  | null -> ()
                  | tab2 -> tab2 |> Seq.to_array |> Array.sortBy (fun kvp -> kvp.Key) |> Array.iter (fun kvp -> colsAcc.Add(kvp.Key); valsAcc.Add(kvp.Value));
        ...
        let listRowVecGU ops xs = mkRowVecGU ops (Array.of_list xs) 
        let listVecGU ops xs = mkVecGU ops (Array.of_list xs) 

        let seqDenseMatrixGU ops xss = listDenseMatrixGU ops (xss |> Seq.to_list |> List.map Seq.to_list)
        let seqVecGU  ops xss = listVecGU ops (xss |> Seq.to_list)
        let seqRowVecGU ops xss = listRowVecGU ops (xss |> Seq.to_list)
        ...
        let inline binaryOpDenseMatrixGU f (a:DenseMatrix<_>) (b:DenseMatrix<_>) = (* pointwise binary operator *)
            ...
            let arrA = a.Values 
            let arrB = b.Values 
            createDenseMatrixGU a.OpsData mA nA (fun i j -> f (getArray2 arrA i j) (getArray2 arrB i j))
        ...
        let inline unaryOpDenseMatrixGU f (a:DenseMatrix<_>) =
            ...
            let mA = a.NumRows 
            let arrA = a.Values 
            createDenseMatrixGU a.OpsData mA nA (fun i j -> f (getArray2 arrA i j))
        ...
        let cptMaxDenseMatrixGU  (a:DenseMatrix<_>) b = let ops = a.ElementOps in binaryOpDenseMatrixGU  max a b
        let cptMinDenseMatrixGU  (a:DenseMatrix<_>) b = let ops = a.ElementOps in binaryOpDenseMatrixGU  min a b
        let cptMaxSparseMatrixGU (a:SparseMatrix<_>) b = let ops = a.ElementOps in binaryOpSparseMatrixGU  max a b
        let cptMinSparseMatrixGU (a:SparseMatrix<_>) b = let ops = a.ElementOps in binaryOpSparseMatrixGU  min a b

        let cptMaxVecGU (a:Vector<_>) b = let ops = a.ElementOps in binaryOpVecGU max a b
        let cptMinVecGU (a:Vector<_>) b = let ops = a.ElementOps in binaryOpVecGU min a b
        ...
        let genericMulDenseMatrix (a:DenseMatrix<_>) (b:DenseMatrix<_>) =
            ...
            createDenseMatrixGU a.OpsData mA nB
              (fun i j -> (0,nA - 1) |> sumRGU ops (fun k -> mul ops (getArray2 arrA i k) (getArray2 arrB k j)))
            ...
            for i = 0 to mA-1 do
                ...
                if kmin1 < kmax1 then 
                    ...
                    if itemp > 0 then 
                        ...
                        Array.sortInPlaceBy (fun (v,idx) -> idx) sorted
        ...
        let mulRowVecDenseMatrixGU (a:RowVector<_>) (b:DenseMatrix<_>) =
            ...
            createRowVecGU a.OpsData nB 
              (fun j -> (0,nA - 1) |> sumRGU ops (fun k -> mul ops arrA.[k] (getArray2 arrB k j)))
        ...
        let mulDenseMatrixVecGU (a:DenseMatrix<_>) (b:Vector<_>) =
            ...
            createVecGU b.OpsData mA
              (fun i -> (0,nA - 1) |> sumRGU ops (fun k -> mul ops (getArray2 arrA i k) arrB.[k]))
        ...
        let mapDenseMatrixGU f (a : DenseMatrix<'T>) : DenseMatrix<'T> = 
            let arrA = a.Values 
            createDenseMatrixGU a.OpsData a.NumRows a.NumCols (fun i j -> f (getArray2 arrA i j))
        ...
        let copyDenseMatrixGU (a : DenseMatrix<'T>) : DenseMatrix<'T> = 
            let arrA = a.Values 
            createDenseMatrixGU a.OpsData a.NumRows a.NumCols (fun i j -> getArray2 arrA i j)

        let copyVecGU (a:Vector<_>) = 
            let mA= a.NumRows
            createVecGU a.OpsData mA (fun i -> a.[i])

        let copyRowVecGU (a:RowVector<_>) = 
            let mA= a.NumCols
            createRowVecGU a.OpsData mA (fun i -> a.[i])

        let to_denseSparseMatrixGU (a:SparseMatrix<_>) = 
            let nA = a.NumCols 
            let mA = a.NumRows
            createDenseMatrixGU a.OpsData mA nA (fun i j -> a.[i,j])
          
        let mapiDenseMatrixGU f (a: DenseMatrix<'T>) : DenseMatrix<'T> = 
            let nA = a.NumCols 
            let mA = a.NumRows
            let arrA = a.Values 
            createDenseMatrixGU a.OpsData mA nA (fun i j -> f i j (getArray2 arrA i j))

        let mapiRowVecGU f (a:RowVector<_>) = 
            let mA= a.NumCols
            createRowVecGU a.OpsData mA (fun i -> f i a.[i])

        let mapiVecGU f (a:Vector<_>) = 
            let mA= a.NumRows
            createVecGU a.OpsData mA (fun i -> f i a.[i])

        let permuteVecGU (p:permutation) (a:Vector<_>) = 
            let mA= a.NumRows 
            createVecGU a.OpsData mA (fun i -> a.[p i])

        let permuteRowVecGU (p:permutation) (a:RowVector<_>) = 
            let mA= a.NumCols 
            createRowVecGU a.OpsData mA (fun i -> a.[p i])

        let inline inplace_mapiDenseMatrixGU f (a:DenseMatrix<_>) = 
            let arrA = a.Values 
            assignDenseMatrixGU (fun i j -> f i j (getArray2 arrA i j)) a
        ...
        let inline foldDenseMatrixGU f z (a:DenseMatrix<_>) =
            ...
            for i = 0 to mA-1 do
                for j = 0 to nA-1 do 
                   acc <- f acc (getArray2 arrA i j)
            acc
        
        let inline foldVecGU f z (a:Vector<_>) =
            let mA = a.NumRows
            let mutable acc = z
            for i = 0 to mA-1 do acc <- f acc a.[i]
            acc
        ...
        let inline foldiDenseMatrixGU f z (a:DenseMatrix<_>) =
            ...
            for i = 0 to mA-1 do
                for j = 0 to nA-1 do 
                   acc <- f i j acc (getArray2 arrA i j)
            acc
        ...
        let sumDenseMatrixGU  (a:DenseMatrix<_>) = 
            let ops = a.ElementOps 
            let zero = ops.Zero 
            foldDenseMatrixGU (fun acc aij -> add ops acc aij) zero a

        let sumSparseMatrixGU  (a:SparseMatrix<_>) = 
            let ops = a.ElementOps 
            let zero = ops.Zero 
            a |> nonZeroEntriesSparseMatrixGU |> Seq.fold (fun acc (_,_,aij) -> add ops acc aij) zero 

        let sumVecGU (a:Vector<_>) = 
            let ops = a.ElementOps 
            let zero = ops.Zero 
            foldVecGU (fun acc ai -> add ops acc ai) zero a

        let prodDenseMatrixGU (a:DenseMatrix<_>) = 
            let ops = a.ElementOps 
            let one = ops.One 
            foldDenseMatrixGU (fun acc aij -> mul ops acc aij) one a

        let prodSparseMatrixGU  (a:SparseMatrix<_>) = a |> to_denseSparseMatrixGU |> prodDenseMatrixGU
        ...
        let inline fold2DenseMatrixGU f z (a:DenseMatrix<_>) (b:DenseMatrix<_>) =
            ...
            for i = 0 to mA-1 do
                for j = 0 to nA-1 do 
                   acc <- f acc (getArray2 arrA i j) (getArray2 arrB i j)
            acc
        ...
        let inplaceAddDenseMatrixGU  (a:DenseMatrix<_>) (b:DenseMatrix<_>) = 
            let ops = a.ElementOps 
            let arrB = b.Values 
            inplace_mapiDenseMatrixGU  (fun i j x -> add ops x (getArray2 arrB i j)) a
        ...
        let inplaceSubDenseMatrixGU  (a:DenseMatrix<_>) (b:DenseMatrix<_>) = 
            let ops = a.ElementOps 
            let arrB = b.Values 
            inplace_mapiDenseMatrixGU  (fun i j x -> sub ops x (getArray2  arrB i j)) a
        ...
        let inplaceCptMulDenseMatrixGU  (a:DenseMatrix<_>) (b:DenseMatrix<_>) = 
            let ops = a.ElementOps 
            let arrB = b.Values 
            inplace_mapiDenseMatrixGU  (fun i j x -> mul ops x (getArray2  arrB i j)) a
        ...
        let inplaceScaleDenseMatrixGU  x (a:DenseMatrix<_>) = 
            let ops = a.ElementOps 
            inplace_mapiDenseMatrixGU  (fun i j y -> ops.Multiply(x,y)) a

        let inplaceScaleVecGU  x (a:Vector<_>) = 
            let ops = a.ElementOps  
            inplace_mapiVecGU  (fun i y   -> ops.Multiply(x,y)) a

        let inplaceScaleRowVecGU x (a:RowVector<_>) = 
            let ops = a.ElementOps 
            inplace_mapiRowVecGU (fun i y   -> ops.Multiply(x,y)) a


        let wrapList (pre,mid,post) show l = 
            match l with 
            | []    -> [pre;post]
            | [x]   -> [pre;show x;post]
        ...
        let primShowDenseMatrixGU (sepX,sepR) (a : DenseMatrix<'e>) =
            ...
            let mA = a.NumRows
            let ops = a.OpsData 
            let showLine i = wrapList ("[",";","]") (showItem ops) ((0,nA-1) |> mapR  (fun j -> a.[i,j])) |> Array.of_list |> System.String.Concat
            wrapList ("matrix [",";"+sepX,"]"+sepR) showLine [0..mA-1] |> Array.of_list |> System.String.Concat
        ...
        let showVecGU s (a : Vector<_>) =
            let mA = a.NumRows
            let ops = a.OpsData 
            wrapList (s+" [",";","]") (showItem ops) ((0,mA-1) |> mapR  (fun i -> a.[i])) |> Array.of_list |> System.String.Concat 
    ...
    module DoubleImpl = 
        ...
        let constDenseMatrixDS  m n x      = GU.createDenseMatrixGU  FloatOps m n (fun i j -> x)
        let constRowVecDS m x              = GU.createRowVecGU FloatOps m   (fun i -> x)
        let constVecDS  m x                = GU.createVecGU  FloatOps m   (fun i -> x)
        ...
        let mulDenseMatrixDS (a:dmatrix) (b:dmatrix) =
            ...
            for i = 0 to mA - 1 do 
                for j = 0 to nB - 1 do 
                    let mutable r = 0.0 
                    for k = 0 to mB - 1 do 
                        r <- r + mul (GU.getArray2 arrA i k) (GU.getArray2 arrB k j)
        ...
        let mulRowVecDenseMatrixDS (a:rowvec) (b:dmatrix) =
            ...
            for j = 0 to nB - 1 do 
                let mutable r = 0.0 
                for k = 0 to mB - 1 do 
                    r <- r + mul arrA.[k] (GU.getArray2 arrB k j)
                arr.[j] <- r
            mkRowVecDS arr
        ...
        let mulDenseMatrixVecDS (a:dmatrix) (b:vector) =
            ...
            for i = 0 to mA - 1 do 
                let mutable r = 0.0 
                for k = 0 to nA - 1 do 
                    r <- r + mul (GU.getArray2 arrA i k) arrB.[k]
                arr.[i] <- r
            mkVecDS arr
        ...
        let traceDenseMatrixDS (a:dmatrix) =
            ...
            let arrA = a.Values 
            (0,nA-1) |> GU.sumfR (fun i -> GU.getArray2 arrA i i) 
        ...
        let inplaceAddDenseMatrixDS  a (b:DenseMatrix<_>) = let arrB = b.Values  in GU.inplace_mapiDenseMatrixGU  (fun i j x -> x + GU.getArray2 arrB i j) a
        ...
        let inplaceSubDenseMatrixDS  a (b:DenseMatrix<_>) = let arrB = b.Values  in GU.inplace_mapiDenseMatrixGU  (fun i j x -> x - GU.getArray2  arrB i j) a
        ...
        let inplaceCptMulDenseMatrixDS  a (b:DenseMatrix<_>) = let arrB = b.Values  in GU.inplace_mapiDenseMatrixGU  (fun i j x -> x * GU.getArray2 arrB i j) a
        ...
        let inplaceScaleDenseMatrixDS  (a:float) b = GU.inplace_mapiDenseMatrixGU  (fun i j x -> a * x) b
        let inplaceScaleVecDS  (a:float) b = GU.inplace_mapiVecGU  (fun i x   -> a * x) b
        let inplace_scaleRowVecDS (a:float) b = GU.inplace_mapiRowVecGU (fun i x   -> a * x) b
    ...
    module SpecializedGenericImpl = 
        ...
        let inline binaryOpM opDenseDS opDenseGU opSparseDS opSparseMatrixGU a b = 
            ...
            | SparseRepr a, DenseRepr b     -> opDenseGU (GU.to_denseSparseMatrixGU a) b         |> dense
            | DenseRepr  a, SparseRepr b    -> opDenseGU a (GU.to_denseSparseMatrixGU b)         |> dense
        ...
        let dotM a b = 
          ...
          | _ ->  
              let ops = a.ElementOps 
              mergedNonZeroEntriesM a b |> Seq.fold (fun z (_,_,va,vb) -> GU.add ops z (GU.mul ops va vb)) ops.Zero 
        ...
        let existsM  f a = 
            match a with 
            | SparseRepr a -> sparseNYI() // note: martin says "run f on a token element if it's not full"
        ...
        let to_denseM a = 
            match a with 
            | SparseRepr a -> GU.to_denseSparseMatrixGU a |> dense
            | DenseRepr _ -> a
        ...
        let compareM (comp: IComparer) (a:Matrix<'T>) (b:Matrix<'T>) = 
            ...
            | DenseRepr a, DenseRepr b -> 
              ...
              go1 0
            | _ -> 
              match (mergedNonZeroEntriesM a b |> Seq.tryPick (fun (i,j,v1,v2) -> let c = comp.Compare(v1,v2) in if c = 0 then None else Some(c))) with
              | None -> 0
              | Some(c) -> c
        ...
        let equalsM (comp: IEqualityComparer) (a:Matrix<'T>) (b:Matrix<'T>) = 
            ...
            | DenseRepr a, DenseRepr b -> 
              ...
              go1 0
            | _ -> 
              mergedNonZeroEntriesM a b |> Seq.forall (fun (i,j,v1,v2) -> comp.Equals(v1,v2))
        ...
        let to_scalarM (x:Matrix<_>) = x.[0,0]
...
//--------------------------------------------------------------------------*)
    ...
    type Matrix<'T> with
        ...
        member m.StructuredDisplayAsArray =  
            let rec layout m = 
                match m with 
                | DenseRepr dm -> box (Array2D.init m.NumRows m.NumCols (fun i j -> m.[i,j]))
                | SparseRepr sm -> (if m.NumRows < 20 && m.NumCols < 20 then layout (SpecializedGenericImpl.to_denseM m) else box(SpecializedGenericImpl.nonZeroEntriesM m))
            layout m
        ...
        interface System.IComparable with 
             member m.CompareTo(yobj:obj) = SpecializedGenericImpl.compareM (LanguagePrimitives.FSharpComparer()) m (yobj :?> Matrix<'T>)
        ...
        override m.GetHashCode() = SpecializedGenericImpl.hashM (LanguagePrimitives.FSharpEqualityComparer()) m 
        override m.Equals(yobj:obj) = 
            match yobj with 
            | :? Matrix<'T> as m2 -> SpecializedGenericImpl.equalsM (LanguagePrimitives.FSharpEqualityComparer()) m m2
            | _ -> false
...
//--------------------------------------------------------------------------*)
    ...
    type Vector<'T> with
        ...
        interface System.IComparable with 
             member m.CompareTo(y:obj) = SpecializedGenericImpl.compareV (LanguagePrimitives.FSharpComparer()) m (y :?> Vector<'T>)
        ...
        override x.GetHashCode() = 
            SpecializedGenericImpl.hashV (LanguagePrimitives.FSharpEqualityComparer()) x

        override x.Equals(yobj) = 
            match yobj with 
            | :? Vector<'T> as v2 -> SpecializedGenericImpl.equalsV (LanguagePrimitives.FSharpEqualityComparer()) x v2
            | _ -> false
...
//--------------------------------------------------------------------------*)
    ...
    type RowVector<'T> with
        ...
        interface System.IComparable with 
            member m.CompareTo(y) = SpecializedGenericImpl.compareRV (LanguagePrimitives.FSharpComparer()) m (y :?> RowVector<'T>)
        ...
        override x.GetHashCode() = 
            SpecializedGenericImpl.hashRV (LanguagePrimitives.FSharpEqualityComparer()) x

        override x.Equals(yobj) = 
            match yobj with 
            | :? RowVector<'T> as rv2 -> SpecializedGenericImpl.equalsRV (LanguagePrimitives.FSharpEqualityComparer()) x rv2
            | _ -> false
    ...
    module Matrix = begin
        ...
        module Generic = begin
            ...
            let of_array2 arr       = MS.arrayM arr
            let to_array2 (m:Matrix<_>) = Array2D.init m.NumRows m.NumCols (fun i j -> get m i j)
            ...
            let inplace_add a b = MS.inplaceAddM a b
            let inplace_sub a b = MS.inplaceSubM a b
            ...
            let compare a b = MS.compareM (LanguagePrimitives.FSharpComparer()) a b
            let hash a = MS.hashM (LanguagePrimitives.FSharpEqualityComparer()) a
        ...
        let of_list    xss   = DS.listDenseMatrixDS    xss |> MS.dense
        let of_seq     xss   = DS.seqDenseMatrixDS    xss |> MS.dense
        ...
        let of_scalar x     = DS.scalarDenseMatrixDS x |> MS.dense
        ...
        let inplace_add  (a:matrix) b = MS.inplaceAddM a b
        let inplace_sub  (a:matrix) b = MS.inplaceSubM a b
...
//--------------------------------------------------------------------------*)
    ...
    module Vector = 
        ...
        module Generic = 
          ...
          let of_list    xss   = OpsS.listV xss
          let of_seq    xss   = OpsS.seqV xss
          let init  m   f = OpsS.initV m f
          let initNumeric  m   f = OpsS.createNumericV m f
          let of_array arr       = OpsS.arrayV arr
          let to_array (v:Vector<_>) = Array.init v.Length (get v)

          let create  m x   = OpsS.constV m x
          let zero n = OpsS.zeroV n
          let ones n = OpsS.createNumericV n (fun ops i -> ops.One)
          let of_scalar   x = OpsS.scalarV x
          ...
          let inplace_add a b = OpsS.inplaceAddV a b
          let inplace_sub a b = OpsS.inplaceSubV a b
        ...
        let of_list    xs    = VecDS.listVecDS    xs
        let of_seq    xs    = VecDS.seqVecDS    xs
        let create  m   x  = VecDS.constVecDS  m   x
        let of_scalar x     = VecDS.scalarVecDS x
...
//--------------------------------------------------------------------------*)
    ...
    module RowVector = 
        ...
        module Generic = 
            ...
            let of_list a        = OpsS.listRV a
            let of_array a        = OpsS.arrayRV a
            let of_seq a         = OpsS.seqRV a
    ...
    type Matrix<'T> with 
        member x.ToArray2()        = Matrix.Generic.to_array2 x
        ...
        member x.ToScalar()        = Matrix.Generic.to_scalar x
        member x.ToRowVector()     = Matrix.Generic.to_rowvec x               
        member x.ToVector()        = Matrix.Generic.to_vector x
    ...
    type Vector<'T> with 
        member x.ToArray() = Vector.Generic.to_array x
        member x.Norm      = Vector.Generic.norm x
        member x.Copy ()   = Vector.Generic.copy x


    type RowVector<'T> with 
        member x.ToArray() = RowVector.Generic.to_array x
...
namespace Microsoft.FSharp.Core
    ...
    module MatrixCommonExtensions = 
        ...
        let matrix ll = Microsoft.FSharp.Math.Matrix.of_seq ll
        let vector l  = Microsoft.FSharp.Math.Vector.of_seq  l
        let rowvec l  = Microsoft.FSharp.Math.RowVector.of_seq l
