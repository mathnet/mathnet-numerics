...
module LinearAlgebraService = 
  ...
  let preDivideByVector A b = 
    let _,_,x  = Service().dgesv_(A,Matrix.of_vector b)
    Matrix.to_vector x
  ...
  let leastSquares a b =
    ...
    let b = if m>n then Matrix.of_vector b else Matrix.init n 1 (fun i j -> if i<m then b.[i] else 0.0)
