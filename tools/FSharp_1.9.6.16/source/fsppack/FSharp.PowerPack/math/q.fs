...
namespace Microsoft.FSharp.Math
    ...
    module BigRationalLargeImpl = 
        ...
        let OneI = new BigInt(1)
        let bigint (x:int) = new BigInt(x)
        let ToDoubleI (x:bigint) = 
#if FX_ATLEAST_40
            (BigInt.op_Explicit x : double)
#else
            BigInt.ToDouble x
#endif
    ...
    type BigRationalLarge = 
        ...
        static member Equals(Q(ap,aq), Q(bp,bq)) = 
            BigInt.(=)  (ap,bp) && BigInt.(=) (aq,bq)   // normal form, so structural equality 
        ...
        member x.IsNegative = let (Q(ap,aq)) = x in sign ap < 0
        member x.IsPositive = let (Q(ap,aq)) = x in sign ap > 0

        member x.Numerator = let (Q(p,q)) = x in p
        member x.Denominator = let (Q(p,q)) = x in q
        member x.Sign = (let (Q(p,q)) = x in sign p)

        static member ToDouble (Q(p,q)) = 
            ToDoubleI p / ToDoubleI q

        static member Normalize (p:BigInt,q:BigInt) =
            if q.IsZero then
...
#if FX_ATLEAST_40
                let k = BigInt.GreatestCommonDivisor(p,q)
#else
                let k = BigInt.Gcd(p,q)
#endif
                let p = p / k 
                let q = q / k 
        ...
        static member Parse (str:string) =
          ...
          let j = str.IndexOf '/' 
          if j >= 0 then 
              let p = BigInt.Parse (str.Substring(0,j)) 
              let q = BigInt.Parse (str.Substring(j+1,len-j-1)) 
              BigRationalLarge.RationalZ (p,q)
          else
              let p = BigInt.Parse str 
...
(*
        static member Floor (Q(p,q)) =
            let d,r = BigInt.DivRem (p,q)          // p = d.q + r 
            if sign r < 0 
            then d - OneI                  // r -ve, so round down 
            else d
        
        static member Ceiling (Q(p,q)) =
           let d,r = BigInt.DivRem (p,q)          // p = d.q + r 
           if sign r > 0 
           then d + OneI                 // when r +ve, adjust up 
           else d
*)
    ...
    module BigRationalLarge = 
...
#if FX_ATLEAST_40
        let pown (Q(p,q)) (n:int) = Q(BigInt.Pow(p,n),BigInt.Pow  (q,n))
#else
        let pow  (Q(p,q)) (z:bigint) = Q(BigInt.Pow(p,z),BigInt.Pow  (q,z))     // p,q powers still coprime 
        let powi q (n:int) = let z = bigint n in pow q z
#endif
        ...
        let lte   (Q(ap,aq)) (Q(bp,bq)) = BigInt.(<=) (ap * bq,bp * aq)
        let gte   (Q(ap,aq)) (Q(bp,bq)) = BigInt.(>=) (ap * bq,bp * aq)
        ...
        // integer part
        let integer (Q(p,q)) =
            let d,r = BigInt.DivRem (p,q)          // have p = d.q + r, |r| < |q| 
            if r < ZeroI
...
(*
        let two = BigInt 2
        let round (Q(p,q)) =
            let d,r = BigInt.DivRem (p,q)             // have p = d.q + r  for  |r|<q 
            let s = sign r  
            if s > 0 then
                if BigInt.(<) (q / two,r) 
                then OneI + d                 // have q/2 < r < q   , round up 
                else d                         // have 0   < r <= q/2, note 1/2 rounds down 
            elif s < 0 then
              if BigInt.(<=) (q / two, -r) 
              then  OneI - d                  // have -1 < r <= q/2 , round down. note 1/2 rounds down again 
...
*)
    ...
    type BigNum =
        ...
        static member PowN (n,i:int) =
            match n with
#if FX_ATLEAST_40
            | Z z -> Z (BigInt.Pow (z,i))
            | Q q -> Q (BigRationalLarge.pown q i)
#else
            | Z z -> Z (BigInt.Pow (z,bigint i))
            | Q q -> Q (BigRationalLarge.powi q i)
#endif

        static member op_Equality (n,nn) = 
            match n,nn with
            | Z z ,Z zz -> BigInt.(=) (z,zz)
...
namespace Microsoft.FSharp.Core
    ...
    module NumericLiteralN = 
        ...
        let FromInt64 (i64:int64) = BigNum.FromBigInt (new BigInt(i64))
