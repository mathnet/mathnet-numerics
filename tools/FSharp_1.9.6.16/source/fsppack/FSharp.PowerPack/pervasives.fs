...
let open_out_encoded (e:Encoding) (s:string) = open_out_gen [Open_text; Open_wronly; Open_creat; Open_encoding e] 777 s
...
let prim_output_newline (os:out_channel) = 
    ...
    | BinaryW bw -> invalidArg "os" "the channel is a binary channel"
...
let output_bytearray (os:out_channel) (b: byte[]) = output os b 0  b.Length
...
let in_channel_to_stream         (x:in_channel) = InChannel.to_Stream x
let in_channel_to_text_reader    (x:in_channel) = InChannel.to_TextReader x
let in_channel_to_stream_reader  (x:in_channel) = InChannel.to_StreamReader x
let in_channel_to_binary_reader  (x:in_channel) = InChannel.to_BinaryReader x
...
let out_channel_to_stream        (x:out_channel) = OutChannel.to_Stream x
let out_channel_to_text_writer   (x:out_channel) = OutChannel.to_TextWriter x
let out_channel_to_stream_writer (x:out_channel) = OutChannel.to_StreamWriter x
let out_channel_to_binary_writer (x:out_channel) = OutChannel.to_BinaryWriter x
