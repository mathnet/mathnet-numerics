...
let usage =
  [ "-o", Arg.String (fun s -> out := Some s), "Name the output file.";
    ...
    "--light", Arg.Unit (fun () ->  light := Some true), "Add #light to the top of the generated file (the default for .fs and .fsi output files)";
    ...
    "--lexlib", Arg.String (fun s ->  lexlib <- s), "Specify the namespace for the implementation of the parser table interperter (default Microsoft.FSharp.Text.Parsing)";
    "--parslib", Arg.String (fun s ->  parslib <- s), "Specify the namespace for the implementation of the parser table interperter (default Microsoft.FSharp.Text.Parsing)";
...
let main() = 
      ...
      try 
        if !tokenize then begin 
          while true do 
            ...
            (*F# printf "tokenize - got %s" (Fsyaccpars.token_to_string t); F#*)
  ...
  let cprintf (os:TextWriter,lineCount) fmt = Printf.twprintf os fmt
  let cprintfn (os:TextWriter,lineCount) fmt = Printf.ktwprintf (fun () -> incr lineCount; os.WriteLine()) os fmt
...
(*
  printfn "#unique rows in action table: %d" (List.length (Array.fold_right (fun row acc -> insert (Array.to_list row) acc) actionTable [])); 
  printfn "maximum #different actions per state: %d" (Array.fold_right (fun row acc ->max (List.length (List.fold_right insert (Array.to_list row) [])) acc) actionTable 0); 
  printfn "average #different actions per state: %d" ((Array.fold_right (fun row acc -> (List.length (List.fold_right insert (Array.to_list row) [])) + acc) actionTable 0) / (Array.length states)); 
*)
  ...
  let types = Map.of_list spec.Types 
  let tokens = Map.of_list spec.Tokens 
...
let _ = 
    try main()
    with e -> 
      printf "Error: %s" (match e with Failure s -> s | e -> e.Message);
      exit 1
