...
let OutputTerminalSet os tset  =
    fprintf os "%s" (String.Join(";", Array.of_seq(tset)))
...
let ProcessParserSpecAst (spec: ParserSpec) = 
    ...
    let prods =  
        spec.Rules |> List.mapi (fun i (nonterm,rules) -> 
            rules |> List.mapi (fun j (Rule(syms,precsym,code)) -> 
                let precInfo = 
                    let precsym = List.fold_right (fun x acc -> match acc with Some _ -> acc | None -> match x with z when IsTerminal z -> Some z | _ -> acc) syms precsym
    ...
    let nonTerminalSet = Set.of_list nonTerminals
...
let LeastFixedPoint f set = 
    let acc = ref set
    ProcessWorkList (Set.to_list set) (fun queueWork item ->
...
type NonTerminalTable(nonTerminals:NonTerminal list) = 
    ...
    let a = Array.of_list nonTerminals
...
type TerminalTable(terminals:(Terminal * PrecedenceInfo) list) = 
    ...
    let a = Array.of_list (List.map fst terminals)
    let b = Array.of_list (List.map snd terminals)
...
type ProductionTable(ntTab:NonTerminalTable, termTab:TerminalTable, nonTerminals:string list, prods: Production list) =
    ...
    let a =  
        ...
        |> Array.of_list
    let b = Array.of_list (List.map (fun (_,Production(nt,_,_,_)) -> ntTab.ToIndex nt) prodsWithIdxs)
    let c = Array.of_list (List.map (fun (_,Production(_,prec,_,_)) -> prec) prodsWithIdxs)
    let productions = 
        nonTerminals
...
type KernelTable(kernels) =
    ...
    let toIdxMap = Map.of_list [ for i,x in kernelsAndIdxs -> x,i ]
    let ofIdxMap = Array.of_list kernels
...
type Closure1Table() = 
    ...
    member table.Add(a,b) = 
        if not (t.ContainsKey(a)) then t.[a] <- new HashSet<_>()
        t.[a].Add(b)
...
type PropagateTable() = 
    ...
    member table.Add(a,b) = 
        if not (t.ContainsKey(a)) then t.[a] <- new HashSet<KernelItemIndex>()
        t.[a].Add(b)
    member table.Item 
      with get(a) = 
...
let CompilerLalrParserSpec logf (spec : ProcessedParserSpec) =
            ...
            let frontier = 
                ...
                for nonTermX in ntTab.Indexes do 
                    for prodIdx in prodTab.Productions.[nonTermX] do
                        let rhs = Array.to_list (prodTab.Symbols prodIdx)
                        let rec place l =
                            match l with
                            | (yi::t) -> 
                                res := 
    ...
    let fakeStartNonTerminalsSet = Set.of_list (fakeStartNonTerminals |> List.map ntTab.ToIndex)
    ...
    let OutputLalrTables os (prods,states, startStates,actionTable,immediateActionTable,gotoTable,endOfInputTerminalIdx,errorTerminalIdx) = 
        let combined = Array.of_list (List.map2 (fun x (y,(z,w)) -> x,y,z,w) (Array.to_list states) (List.zip (Array.to_list actionTable) (List.zip (Array.to_list immediateActionTable) (Array.to_list gotoTable))))
        fprintfn os "------------------------";
        fprintfn os "states = ";
        fprintfn os "%a" OutputCombined combined;
        fprintfn os "startStates = %s" (String.Join(";",Array.of_list (List.map string startStates)));
        fprintfn os "------------------------"
    ...
    let ComputeClosure0NonTerminal = 
        Memoize (fun nt -> 
            let seed = (List.fold_right (prodIdx_to_item0 >> Set.add) prodTab.Productions.[nt] Set.empty)
            LeastFixedPoint 
                (fun item0 -> 
    ...
    let kernels = 
        ...
        !acc |> Seq.to_list |> List.map (Set.filter IsKernelItem)
        ...
        ProcessWorkList iset (fun addToWorkList (item0,pretokens:Set<TerminalIndex>) ->
            pretokens |> Set.iter (fun pretoken -> 
                if not (acc.Contains(item0,pretoken)) then
                    acc.Add(item0,pretoken)
                    let rsyms = rsyms_of_item0 item0 
                    if rsyms.Length > 0 then 
                        match rsyms.[0] with 
                        | (PNonTerminal ntB) -> 
                             let firstSet = ComputeFirstSetOfTokenList (Array.to_list rsyms.[1..],pretoken)
    ...
    let spontaneous, propagate  =
        let closure1OfItem0WithDummy = 
            Memoize (fun item0 -> ComputeClosure1 [(item0,Set.of_list [dummyLookaheadIdx])])
                    ...
                    | Some rsym ->
                         ...
                         | Some gotoKernelIdx ->
                              ...
                              for lookaheadToken in lookaheadTokens do
                                  ...
                                  then propagate.Add(item0Idx, gotoItemIdx)
                                  else spontaneous.Add(gotoItemIdx, lookaheadToken)
    ...
    let actionTable, immediateActionTable = 
        ...
        Array.of_list (List.map fst actionInfo),
        Array.of_list (List.map snd actionInfo)

    // The goto table is much simpler - it is based on LR(0) kernels alone. 

    reportTime(); printf  "building goto table..."; stdout.Flush();
    let gotoTable = 
         let gotos kernelIdx = Array.of_list (List.map (fun nt -> gotoKernel (GotoItemIdx(kernelIdx,PNonTerminal nt))) ntTab.Indexes)
         Array.of_list (List.map gotos kernelTab.Indexes)
    ...
    /// The final results
    let states = kernels |> Array.of_list 
    let prods = Array.of_list (List.map (fun (Production(nt,prec,syms,code)) -> (nt, ntTab.ToIndex nt, syms,code)) prods)
    ...
    let states = states |> Array.map (Set.to_list >> List.map prodIdx_of_item0)
